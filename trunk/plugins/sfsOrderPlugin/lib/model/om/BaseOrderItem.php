...
abstract class BaseOrderItem extends BaseObject  implements Persistent {
	...
	public function setDeliveryCost($v)
	{
		...
		if ($this->delivery_cost !== $v || $v === 0) {
			$this->delivery_cost = $v;
			$this->modifiedColumns[] = OrderItemPeer::DELIVERY_COST;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->member_first_name = $rs->getString($startcol + 3);

			$this->member_last_name = $rs->getString($startcol + 4);

			$this->member_country_id = $rs->getInt($startcol + 5);

			$this->member_state_id = $rs->getInt($startcol + 6);

			$this->member_state_title = $rs->getString($startcol + 7);

			$this->member_city = $rs->getString($startcol + 8);

			$this->member_street = $rs->getString($startcol + 9);

			$this->member_postcode = $rs->getString($startcol + 10);

			$this->billing_first_name = $rs->getString($startcol + 11);

			$this->billing_last_name = $rs->getString($startcol + 12);

			$this->billing_country_id = $rs->getInt($startcol + 13);

			$this->billing_state_id = $rs->getInt($startcol + 14);

			$this->billing_state_title = $rs->getString($startcol + 15);

			$this->billing_city = $rs->getString($startcol + 16);

			$this->billing_street = $rs->getString($startcol + 17);

			$this->billing_postcode = $rs->getString($startcol + 18);

			$this->delivery_first_name = $rs->getString($startcol + 19);

			$this->delivery_last_name = $rs->getString($startcol + 20);

			$this->delivery_country_id = $rs->getInt($startcol + 21);

			$this->delivery_state_id = $rs->getInt($startcol + 22);

			$this->delivery_state_title = $rs->getString($startcol + 23);

			$this->delivery_city = $rs->getString($startcol + 24);

			$this->delivery_street = $rs->getString($startcol + 25);

			$this->delivery_postcode = $rs->getString($startcol + 26);

			$this->delivery_cost = $rs->getFloat($startcol + 27);

			$this->delivery_description = $rs->getString($startcol + 28);

			$this->status_id = $rs->getInt($startcol + 29);
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[3] => $this->getMemberFirstName(),
			$keys[4] => $this->getMemberLastName(),
			$keys[5] => $this->getMemberCountryId(),
			$keys[6] => $this->getMemberStateId(),
			$keys[7] => $this->getMemberStateTitle(),
			$keys[8] => $this->getMemberCity(),
			$keys[9] => $this->getMemberStreet(),
			$keys[10] => $this->getMemberPostcode(),
			$keys[11] => $this->getBillingFirstName(),
			$keys[12] => $this->getBillingLastName(),
			$keys[13] => $this->getBillingCountryId(),
			$keys[14] => $this->getBillingStateId(),
			$keys[15] => $this->getBillingStateTitle(),
			$keys[16] => $this->getBillingCity(),
			$keys[17] => $this->getBillingStreet(),
			$keys[18] => $this->getBillingPostcode(),
			$keys[19] => $this->getDeliveryFirstName(),
			$keys[20] => $this->getDeliveryLastName(),
			$keys[21] => $this->getDeliveryCountryId(),
			$keys[22] => $this->getDeliveryStateId(),
			$keys[23] => $this->getDeliveryStateTitle(),
			$keys[24] => $this->getDeliveryCity(),
			$keys[25] => $this->getDeliveryStreet(),
			$keys[26] => $this->getDeliveryPostcode(),
			$keys[27] => $this->getDeliveryCost(),
			$keys[28] => $this->getDeliveryDescription(),
			$keys[29] => $this->getStatusId(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setMemberFirstName($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setMemberLastName($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setMemberCountryId($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setMemberStateId($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setMemberStateTitle($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setMemberCity($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setMemberStreet($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setMemberPostcode($arr[$keys[10]]);
		if (array_key_exists($keys[11], $arr)) $this->setBillingFirstName($arr[$keys[11]]);
		if (array_key_exists($keys[12], $arr)) $this->setBillingLastName($arr[$keys[12]]);
		if (array_key_exists($keys[13], $arr)) $this->setBillingCountryId($arr[$keys[13]]);
		if (array_key_exists($keys[14], $arr)) $this->setBillingStateId($arr[$keys[14]]);
		if (array_key_exists($keys[15], $arr)) $this->setBillingStateTitle($arr[$keys[15]]);
		if (array_key_exists($keys[16], $arr)) $this->setBillingCity($arr[$keys[16]]);
		if (array_key_exists($keys[17], $arr)) $this->setBillingStreet($arr[$keys[17]]);
		if (array_key_exists($keys[18], $arr)) $this->setBillingPostcode($arr[$keys[18]]);
		if (array_key_exists($keys[19], $arr)) $this->setDeliveryFirstName($arr[$keys[19]]);
		if (array_key_exists($keys[20], $arr)) $this->setDeliveryLastName($arr[$keys[20]]);
		if (array_key_exists($keys[21], $arr)) $this->setDeliveryCountryId($arr[$keys[21]]);
		if (array_key_exists($keys[22], $arr)) $this->setDeliveryStateId($arr[$keys[22]]);
		if (array_key_exists($keys[23], $arr)) $this->setDeliveryStateTitle($arr[$keys[23]]);
		if (array_key_exists($keys[24], $arr)) $this->setDeliveryCity($arr[$keys[24]]);
		if (array_key_exists($keys[25], $arr)) $this->setDeliveryStreet($arr[$keys[25]]);
		if (array_key_exists($keys[26], $arr)) $this->setDeliveryPostcode($arr[$keys[26]]);
		if (array_key_exists($keys[27], $arr)) $this->setDeliveryCost($arr[$keys[27]]);
		if (array_key_exists($keys[28], $arr)) $this->setDeliveryDescription($arr[$keys[28]]);
		if (array_key_exists($keys[29], $arr)) $this->setStatusId($arr[$keys[29]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(OrderItemPeer::DELIVERY_COST)) $criteria->add(OrderItemPeer::DELIVERY_COST, $this->delivery_cost);
