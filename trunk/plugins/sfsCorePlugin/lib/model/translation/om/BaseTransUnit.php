...
abstract class BaseTransUnit extends BaseObject  implements Persistent {
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->date_added = $rs->getInt($startcol + 5);

			$this->date_modified = $rs->getInt($startcol + 6);

			$this->author = $rs->getString($startcol + 7);

			$this->translated = $rs->getBoolean($startcol + 8);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[8], $arr)) $this->setTranslated($arr[$keys[8]]);
	}
