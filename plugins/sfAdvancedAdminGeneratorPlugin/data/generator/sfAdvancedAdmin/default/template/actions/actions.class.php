  ...
  public function handlePost($type)
  {
    ...
    catch (PropelException $e)
    {
      if ( $type == 'edit' ) {
        ...
        $this->getRequest()->setError('create', 'Could not save the created <?php echo sfInflector::humanize($this->getPluralName()) ?>.');
      }
      return $this->forward('<?php echo $this->getModuleName() ?>', 'list');
    }
...
<?php endforeach; ?>
  ...
  {
    if (method_exists('<?php echo $this->getClassName() ?>', 'getIsDeleted')) {
        $<?php echo $this->getSingularName() ?>->setIsDeleted(1);
        $<?php echo $this->getSingularName() ?>->save();
    }
    else {
...
<?php endif; ?>
  ...
  // add by nest
  if (method_exists('<?php echo $this->getClassName() ?>Peer', 'addAdminCriteria')) {
      <?php echo $this->getClassName() ?>Peer::addAdminCriteria($c);
  }
  // end
...
<?php endforeach; ?>
  ...
  protected function getMaps()
  {
   ...
   {
      sfToolKit::clearGlob(sfConfig::get('sf_cache_dir') . '/frontend/*/template/');
   }
   //end
}
