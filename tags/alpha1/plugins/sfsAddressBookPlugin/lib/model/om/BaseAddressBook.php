...
abstract class BaseAddressBook extends BaseObject  implements Persistent {
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->country_id = $rs->getInt($startcol + 5);

			$this->state_id = $rs->getInt($startcol + 6);

			$this->state_title = $rs->getString($startcol + 7);

			$this->city = $rs->getString($startcol + 8);

			$this->street = $rs->getString($startcol + 9);

			$this->postcode = $rs->getString($startcol + 10);

			$this->is_default = $rs->getBoolean($startcol + 11);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[7], $arr)) $this->setStateTitle($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setCity($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setStreet($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setPostcode($arr[$keys[10]]);
